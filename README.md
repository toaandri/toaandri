
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,50:202020,100:ffffff&height=180&section=header&text=TOAANDRI&fontSize=46&fontColor=ffffff&animation=fadeIn&fontAlignY=38" width="100%" alt="Animated monochrome profile header" />
</p>

<h1 align="center">ANDRIANARIJERY Toaviniaina Maharavo</h1>

<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=20&duration=2800&pause=900&color=111111&center=true&vCenter=true&width=650&lines=SOFTWARE+%26+WEB+DEVELOPER;Building+useful+digital+experiences;React+%C2%B7+Node.js+%C2%B7+TypeScript" alt="Animated introduction" />
</p>

<br/>

<p align="center">
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=toaandri&theme=default" width="48%" alt="GitHub statistics" />
  <img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=toaandri&theme=default" width="48%" alt="Top languages" />
</p>
<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=toaandri&hide_border=true&background=ffffff&stroke=111111&ring=111111&fire=111111&currStreakLabel=111111&sideLabels=444444&currStreakNum=111111&sideNums=111111&dates=666666" width="70%" alt="GitHub contribution streak" />
</p>

---

## About Me

> **Software & web developer** based in Antananarivo, Madagascar. I design and build interactive frontends, complex backends, and robust database architectures with a focus on clean, maintainable code.

- Full-stack development with **React, Node.js, TypeScript** and modern web technologies
- Mobile applications with **React Native** and Expo
- REST, GraphQL and real-time systems with WebSockets
- Open to collaborations, internships and interesting projects

---

## Services

| Area | Focus |
|------|-------|
| **Full-stack web** | React frontends, Node.js backends and production-ready platforms |
| **Mobile apps** | Cross-platform applications with React Native and Expo |
| **Backend & APIs** | REST, GraphQL, WebSockets, authentication and scalable services |
| **Database architecture** | PostgreSQL, MongoDB, Redis, Firebase and Supabase |

---

## Tech Stack

<p align="center">
  <img src="https://skillicons.dev/icons?i=ts,js,java,python,php,react,html,css,tailwind,nodejs,nestjs,express,graphql,postgres,mongodb,redis,firebase,supabase,docker,kubernetes,aws,vercel,git,github,githubactions,jest,vitest,cypress,playwright&perline=10&theme=light" alt="Technology stack" />
</p>

---

## Featured Projects

<p align="center"><sub>Selected from my pinned GitHub repositories</sub></p>

<table>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/toaandri/Anamboatra">Anamboatra</a></h3>
      <p>End-of-year L2 project: a full-stack platform for citizens, staff and field teams with real-time updates.</p>
      <img src="https://img.shields.io/badge/TypeScript-111111?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/toaandri/M-E-kaly">M-E-kaly</a></h3>
      <p>Online food ordering and delivery system designed for the Malagasy market.</p>
      <img src="https://img.shields.io/badge/TypeScript-111111?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
    </td>
  </tr>
  <tr>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/toaandri/MITANEKO">MITANEKO</a></h3>
      <p>Application created for the Orange Digital Center POESAM program.</p>
      <img src="https://img.shields.io/badge/TypeScript-111111?style=flat-square&logo=typescript&logoColor=white" alt="TypeScript" />
    </td>
    <td width="50%" valign="top">
      <h3><a href="https://github.com/toaandri/Stick-balancing">Stick-balancing</a></h3>
      <p>Reinforcement learning project focused on balancing a stick vertically.</p>
      <img src="https://img.shields.io/badge/Python-111111?style=flat-square&logo=python&logoColor=white" alt="Python" />
    </td>
  </tr>
</table>

## Let's Connect

<p align="center">
  <strong>Have a project in mind?</strong><br/>
  Let's build something useful together.
</p>

<p align="center">
  <a href="https://toaandri.vercel.app"><img src="https://img.shields.io/badge/PORTFOLIO-111111?style=for-the-badge&logo=vercel&logoColor=white" alt="Portfolio" /></a>
  <a href="https://github.com/toaandri"><img src="https://img.shields.io/badge/GITHUB-111111?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
  <a href="https://www.linkedin.com/in/maharavo"><img src="https://img.shields.io/badge/LINKEDIN-111111?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://web.facebook.com/ravo.mah"><img src="https://img.shields.io/badge/FACEBOOK-111111?style=for-the-badge&logo=facebook&logoColor=white" alt="Facebook" /></a>
  <a href="mailto:toavinamaharavo@gmail.com?subject=Hello%20Toaviniaina"><img src="https://img.shields.io/badge/EMAIL-111111?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
</p>

---

<p align="center"><em>Misaotra anao nitsidika eto! (Thanks for stopping by!)</em></p>

<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:ffffff,50:777777,100:000000&height=100&section=footer" width="100%" alt="Monochrome animated footer" />
</p>
